...
import { withdrawamount } from "../services/admin_api";

const WithdrawForm = ({ closeform, showalert }) => {
  ...
  const { values, errors, setErrors,handleBlur, handleChange, handleSubmit } = useFormik({
    ...
    onSubmit: async (values) => {
      setLoading(true);
      const response=await withdrawamount(values);
  ...
  useEffect(()=>{
    ...
    else if(msg==="withdraw successfully"){
      closeform();
  ...
  return (
    <div className="withdraw">
      <form className="withdraw-form" onSubmit={handleSubmit}>
        <h3 className="text-center">withdraw amount</h3>
        <div className="mb-3">
